  ...
  for (unsigned int i=0; i<numEngines; i++) {
    switch (Engines[i]->GetType()) {
      case FGEngine::etPiston:
        static_cast<FGPiston*>(Engines[i])->ResetToIC();
        try {
      ...
      case FGEngine::etTurbine:
        static_cast<FGTurbine*>(Engines[i])->ResetToIC();
        try {
          if (HasInitializedEngines && (InitializedEngines & i)) InitRunning(i);
        } catch (string str) {
          cerr << str << endl;
...
void FGPropulsion::SetCutoff(int setting)
{
  if (ActiveEngine < 0) {
    for (unsigned i=0; i<Engines.size(); i++) {
        if(Engines[i]->GetType() == FGEngine::etTurbine)
            static_cast<FGTurbine*>(Engines[i])->SetCutoff(setting != 0);
    }
  } else {
    if(Engines[ActiveEngine]->GetType() == FGEngine::etTurbine)
        static_cast<FGTurbine*>(Engines[ActiveEngine])->SetCutoff(setting != 0);
  }
}
